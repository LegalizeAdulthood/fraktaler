...
// text-overlay shader ported from zoomasm:
...
#if 1
const float ZoomOffset = 0.0;
const vec3 TextBorder = vec3(0.0);
const float TextBorderWidth = 0.2;
const vec3 TextColour = vec3(1.0);
const int Decimals = 3;
const float HorizontalAlign = 1.0;
...
int[SCIENTIFIC_MAX] formatScientific(float number, int decimals)
{
  ...
  {
    return formatScientificLog2(number > 0.0 ? 1 : -1, log2(abs(number)), decimals);
  }
}
...
vec3 colour(void)
{
  ...
  vec3 RGB = getRGB();
  vec2 Coord = getCoord();
  vec2 ImageSize = vec2(getImageSize());

  // position near the top right of 16:9 frame
  vec2 p = ((Coord / vec2(ImageSize)) * vec2(16.0, 16.0) + vec2(-16.0, -15.0)) * 2.0;
  vec2 dx = vec2(1.0 / ImageSize.x * 16.0 * 2.0, 0.0);
  vec2 dy = vec2(0.0, 1.0 / ImageSize.y * 16.0 * 2.0);
  float value = getZoomLog2() - ZoomOffset;

  // draw number distance field
  float dist = 1.0/0.0;
  dist = min(dist, sdDigits(p - vec2(-1.0, 0.0), formatScientificLog2(1, value, Decimals), HorizontalAlign, StrokeLength, StrokeWidth, GlyphScale));
